...
export default function useAudioReactive(musicSectionRef, playingRef) {
  ...
  useEffect(() => {
    ...
    function tick() {
      ...
      let tEnergy = 0, tBass = 0, tMid = 0, tHigh = 0
      ...
      if (isPlaying) {
        ...
        tMid = 0.35 + 0.2 * Math.sin(t * 3.2 + 1.0) + 0.15 * Math.sin(t * 5.8 + 2.1) + 0.1 * Math.sin(t * 1.4)

        tHigh =
          0.2 + 0.15 * Math.sin(t * 7.3 + 0.8) + 0.1 * Math.sin(t * 11.2 + 1.5) + 0.08 * Math.sin(t * 2.9)
