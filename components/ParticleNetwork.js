  ...
  useEffect(() => {
    ...
    function createParticles() {
      ...
      for (let i = 0; i < PARTICLE_COUNT; i++) {
        ...
        const sizeBonus = shape === 'circle' ? 0 : shape === 'star' ? 1.5 : shape === 'hexagon' ? 1.2 : 0.6
    ...
    function tick() {
      ...
      for (const p of particles) {
        ...
        const twistForce = scrollVel * 0.008 * (p.shape !== 'circle' ? 1.5 : 0.5)
      ...
      for (const p of visible) {
        ...
        if (streakLen > 1.5) {
          ...
          const trailGrad = ctx.createLinearGradient(p.x, screenY, p.x, trailEndY)
          trailGrad.addColorStop(
            0,
            `hsla(${p.hue}, ${sat}%, ${nodeLightness}%, ${p.alpha * nodeAlphaScale * 0.6})`
          )
          trailGrad.addColorStop(1, `hsla(${p.hue}, ${sat}%, ${nodeLightness}%, 0)`)
          ctx.beginPath()
        ...
        const glowAlpha = (p.alpha + (p.shape !== 'circle' ? scrollGlow : scrollGlow * 0.3)) * nodeAlphaScale
        ...
        if (p.r > 2 || p.shape !== 'circle') {
          ...
          const glowA = (baseGlowA + scrollGlow * 0.5) * p.alpha * nodeAlphaScale
          ctx.beginPath()
          ctx.arc(p.x, screenY, glowR, 0, Math.PI * 2)
          const grad = ctx.createRadialGradient(p.x, screenY, 0, p.x, screenY, glowR)
          grad.addColorStop(0, `hsla(${p.hue}, ${glowSat}%, ${glowL}%, ${glowA})`)
